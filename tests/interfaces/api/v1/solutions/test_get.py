...
async def request_get_solution(client: AsyncClient):
...
async def test__success__get_own_solution(uow, request_get_solution):
    ...
    response = await request_get_solution(solution_id=solution.id, token=token)
    assert response.status_code == status.HTTP_200_OK


async def test__failed__get_other_solution(uow, request_get_solution):
    user = (await create_users(uow))[0]
    workspace, task = await create_workspace_with_task(uow, user.id)
    ...
    response = await request_get_solution(solution_id=solution.id, token=other_token)
    assert response.status_code == status.HTTP_403_FORBIDDEN


async def test__failed__get_solution_not_found(uow, request_get_solution):
    user = (await create_users(uow))[0]
    token = create_access_token(user.as_short())

    response = await request_get_solution(solution_id=99999, token=token)
