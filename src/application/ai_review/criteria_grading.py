...
"""
    ...
    with open(ROOT_DIR / "examples" / "project_docs" / "avito-price-prediction" / "final_project_doc.md", "r") as f:
        project_doc = f.read()
...
async def grade_by_codebase(
    ...
    criteria = [
        CriterionWithCommentsDTO(
            id=66,
            description="""Для проекта должен быть написан docker-compose.yaml, которого достаточно для запуска всей системы""",
            comments=["В структуре проекта и описании зависимостей отсутствует упоминание файла docker-compose.yaml. Также не указано, что проект упакован в Docker-контейнеры или может быть запущен через Docker. Архитектура предполагает запуск скриптов локально (main.py), без контейнеризации. Вероятно, файл docker-compose.yaml отсутствует. Для проверки требуется анализ корневой директории проекта.",]
        ),
    ]
    with open(ROOT_DIR / "examples" / "project_docs" / "avito-price-prediction" / "project_content.md", "r") as f:
        project_content_doc = f.read()

    with open(ROOT_DIR / "examples" / "project_docs" / "avito-price-prediction" / "project_tree.md", "r") as f:
        project_tree_doc = f.read()

    system_content = prompt_builder.build(
        prompt_path="criteria_checks/codebase_grading/system.tpl",
        criteria=criteria
    )
    user_content = prompt_builder.build(
