...
async def get_wind_rose(
    ...
    async with uow.connection():
        ...
        task_to_criterion = {tc.id: tc.criterion_id for tc in task_criteria}

        criterion_ids = list(task_to_criterion.values())
        criteria = await uow.criteria.get_list(CriterionFiltersDTO(ids=criterion_ids))
        criteria_tags = {c.id: c.tags for c in criteria}

        tag_stats: dict[str, dict[str, int]] = defaultdict(lambda: {"passed": 0, "count": 0})

        for check in checks:
            criterion_id = task_to_criterion.get(check.task_criterion_id)
            if not criterion_id:
                continue
        ...
        return [
            WindRosePointDTO(
                tag=tag,
                value=stats["passed"] / stats["count"] if stats["count"] else 0.0,
                count=stats["count"],
            )
