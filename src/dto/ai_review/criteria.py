...
from src.constants.ai_review import CriterionStatusEnum, CriterionStageEnum
...
class CriterionCheckDTO(BaseDTO):
    ...
    status: CriterionStatusEnum = Field(description="Можно ли на основании данных точно сказать, выполнен ли критерий, или требуется дополнительная информация")
    is_passed: bool | None = Field(default=None, description="Факт выполнения критерия (имеет смысл только при status=SUFFICIENT)")
...
class CriterionCheckVersionDTO(BaseDTO):
    ...
    comment: str = Field(description="Комментарий о соблюдении критерии по представленным сведениям")
    status: CriterionStatusEnum = Field(description="Статус проверки критерия")
