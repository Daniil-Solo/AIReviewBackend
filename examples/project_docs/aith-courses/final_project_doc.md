...
│   │   │   ├── courses/        # Кэширование курсов
│   │   │   └── feedback/       # Кэширование отзывов
...
erDiagram
    ...
    courses ||--o{ course_runs_ : "запуски"
    ...
    course_runs_ ||--o{ playlists : "плейлисты"
...
- `POST /api/v1/auth/logout` - Выход из системы (src/api/auth/router.py)
  - Параметры: auth_token (в заголовке)
  - Схема ответа: `SuccessResponse`
  - Назначение: Завершение сессии пользователя

- `GET /api/v1/auth/me` - Получение текущего пользователя (src/api/auth/router.py)
  - Параметры: auth_token (в заголовке)
  - Схема ответа: `UserDTO`
  - Назначение: Получение информации о текущем пользователе

**Курсы:**
- `GET /api/v1/courses` - Получение всех курсов (src/api/courses/router.py)
...
- `GET /api/v1/courses/{course_id}` - Получение курса (src/api/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `CourseFullDTO`
  - Назначение: Получение полной информации о курсе

- `GET /api/v1/courses/{course_id}/favorite_status` - Статус избранного (src/api/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `CourseFavoriteStatusResponse`
  - Назначение: Проверка, добавлен ли курс в избранное

- `POST /api/v1/admin/courses` - Создание курса (src/api/admin/courses/router.py)
  - Параметры: name
...
- `PUT /api/v1/admin/courses/{course_id}` - Обновление курса (src/api/admin/courses/router.py)
  - Параметры: course_id
  - Схема запроса: `UpdateCourseRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Обновление информации о курсе (только для администраторов)

- `DELETE /api/v1/admin/courses/{course_id}` - Удаление курса (src/api/admin/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление курса (только для администраторов)

- `POST /api/v1/admin/courses/{course_id}/published` - Публикация курса (src/api/admin/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Публикация курса (только для администраторов)

- `DELETE /api/v1/admin/courses/{course_id}/published` - Скрытие курса (src/api/admin/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Скрытие курса (только для администраторов)

**Отзывы:**
- `GET /api/v1/courses/{course_id}/feedbacks` - Получение отзывов (src/api/feedback/router.py)
  - Параметры: course_id
  - Схема ответа: `list[FeedbackDTO]`
  - Назначение: Получение отзывов о курсе

- `POST /api/v1/courses/{course_id}/feedbacks` - Создание отзыва (src/api/feedback/router.py)
  - Параметры: course_id
  - Схема запроса: `CreateFeedbackRequest`
  - Схема ответа: `CreateFeedbackResponse`
  - Назначение: Создание отзыва о курсе

- `DELETE /api/v1/courses/{course_id}/feedbacks/{feedback_id}` - Удаление отзыва (src/api/feedback/router.py)
  - Параметры: course_id, feedback_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление отзыва (только автором)

- `POST /api/v1/courses/{course_id}/feedbacks/{feedback_id}/vote` - Голосование за отзыв (src/api/feedback/router.py)
  - Параметры: course_id, feedback_id
  - Схема запроса: `VoteDTO`
  - Схема ответа: `SuccessResponse`
  - Назначение: Оценка отзыва (like/dislike)

- `DELETE /api/v1/courses/{course_id}/feedbacks/{feedback_id}/vote` - Отмена оценки (src/api/feedback/router.py)
  - Параметры: course_id, feedback_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Отмена оценки отзыва

**Профили талантов:**
- `GET /api/v1/talent/profile` - Получение профиля (src/api/talent_profile/router.py)
  - Схема ответа: `TalentProfileDTO`
  - Назначение: Получение профиля текущего пользователя

- `PUT /api/v1/talent/profile/general` - Обновление профиля (src/api/talent_profile/router.py)
  - Схема запроса: `ProfileGeneralUpdateRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Обновление основной информации профиля

- `PUT /api/v1/talent/profile/links` - Обновление ссылок (src/api/talent_profile/router.py)
  - Схема запроса: `ProfileLinksUpdateRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Обновление ссылок в профиле

- `POST /api/v1/talent/profile` - Создание профиля (src/api/talent_profile/router.py)
  - Схема ответа: `SuccessResponse`
  - Назначение: Создание профиля для пользователя

**Избранные курсы:**
- `GET /api/v1/talent/profile/favorites` - Получение избранных (src/api/favorite_courses/router.py)
  - Схема ответа: `list[FavoriteCourseDTO]`
  - Назначение: Получение списка избранных курсов

- `POST /api/v1/talent/profile/favorites` - Добавление в избранное (src/api/favorite_courses/router.py)
  - Схема запроса: `AddFavoriteCourseRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Добавление курса в избранное

- `DELETE /api/v1/talent/profile/favorites/{favorite_course_id}` - Удаление из избранного (src/api/favorite_courses/router.py)
  - Параметры: favorite_course_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление курса из избранного

**Расписания:**
- `GET /api/v1/courses/{course_id}/timetable` - Получение расписания (src/api/timetable/router.py)
  - Параметры: course_id
  - Схема ответа: `TimetableDTO`
  - Назначение: Получение актуального расписания курса

- `GET /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable` - Получение расписания (админ) (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id
  - Схема ответа: `TimetableDTO`
  - Назначение: Получение расписания запуска курса (только для администраторов)

- `POST /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/rules` - Создание правила (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id
...
- `PUT /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/rules/{rule_id}` - Обновление правила (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id, rule_id
  - Схема запроса: `CreateOrUpdateRuleRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Обновление правила расписания (только для администраторов)

- `DELETE /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/rules/{rule_id}` - Удаление правила (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id, rule_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление правила расписания (только для администраторов)

**Google Calendar группы:**
- `GET /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/google_calendar_groups` - Получение групп (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id
  - Схема ответа: `list[GroupGoogleCalendarDTO]`
  - Назначение: Получение групп Google Calendar для запуска курса

- `POST /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/google_calendar_groups` - Создание группы (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id
  - Схема запроса: `CreateGroupGoogleCalendarRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Создание группы Google Calendar для запуска курса

- `DELETE /api/v1/admin/courses/{course_id}/runs/{course_run_id}/timetable/google_calendar_groups/{group_google_calendar_id}` - Удаление группы (src/api/admin/timetable/router.py)
  - Параметры: course_id, course_run_id, group_google_calendar_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление группы Google Calendar

**Плейлисты:**
- `GET /api/v1/courses/{course_id}/playlists` - Получение плейлистов (src/api/playlists/router.py)
  - Параметры: course_id
  - Схема ответа: `list[PlaylistDTO]`
  - Назначение: Получение плейлистов для курса

- `GET /api/v1/admin/courses/{course_id}/runs/{course_run_id}/playlists` - Получение плейлистов (админ) (src/api/admin/playlists/router.py)
  - Параметры: course_id, course_run_id
  - Схема ответа: `list[PlaylistDTO]`
  - Назначение: Получение плейлистов для запуска курса (только для администраторов)

- `POST /api/v1/admin/courses/{course_id}/runs/{course_run_id}/playlists` - Создание плейлиста (src/api/admin/playlists/router.py)
  - Параметры: course_id, course_run_id
  - Схема запроса: `CreateOrUpdatePlaylistRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Создание плейлиста для запуска курса

- `PUT /api/v1/admin/courses/{course_id}/runs/{course_run_id}/playlists/{playlist_id}` - Обновление плейлиста (src/api/admin/playlists/router.py)
  - Параметры: course_id, course_run_id, playlist_id
  - Схема запроса: `CreateOrUpdatePlaylistRequest`
  - Схема ответа: `SuccessResponse`
  - Назначение: Обновление плейлиста для запуска курса

- `DELETE /api/v1/admin/courses/{course_id}/runs/{course_run_id}/playlists/{playlist_id}` - Удаление плейлиста (src/api/admin/playlists/router.py)
  - Параметры: course_id, course_run_id, playlist_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление плейлиста для запуска курса

**Запуски курсов:**
- `GET /api/v1/admin/courses/{course_id}/runs` - Получение запусков (src/api/admin/courses/router.py)
  - Параметры: course_id
  - Схема ответа: `list[CourseRunDTO]`
  - Назначение: Получение запусков курса (только для администраторов)

- `POST /api/v1/admin/courses/{course_id}/runs` - Создание запуска (src/api/admin/courses/router.py)
  - Параметры: course_id
...
- `DELETE /api/v1/admin/courses/{course_id}/runs/{course_run_id}` - Удаление запуска (src/api/admin/courses/router.py)
  - Параметры: course_id, course_run_id
  - Схема ответа: `SuccessResponse`
  - Назначение: Удаление запуска курса (только для администраторов)

**Интеграции:**
- `POST /api/v1/integrations/google_calendar_links` - Обновление ссылок Google Calendar (src/api/admin/integrations/router.py)
...
**Мониторинг:**
- `GET /api/v1/health_check` - Проверка работоспособности (src/api/health_check.py)
  - Схема ответа: `Health`
...
Конфигурация приложения определена в `src/config.py` в классе `Config`, который использует `pydantic-settings` для загрузки переменных из `.env` файла.

### Основные константы и конфиги приложения
- `ApplicationMode`: перечисление режимов работы (DEV, PRODUCTION, TEST)
- `TIME_TO_LIVE_AUTH_SESSION`: время жизни сессии аутентификации (60 * 60 * 24 секунд), определено в `src/domain/auth/constants.py`
- `PASSWORD_MIN_LENGTH`: минимальная длина пароля (8 символов)
- `TIME_TO_LIVE_ALL_COURSES`: время жизни кэша всех курсов (60 * 60 секунд), определено в `src/infrastructure/redis/courses/constants.py`
- `TIME_TO_LIVE_ONE_COURSE`: время жизни кэша одного курса (24 * 60 * 60 секунд), определено в `src/infrastructure/redis/courses/constants.py`
- `TIME_TO_LIVE_FEEDBACKS`: время жизни кэша отзывов (60 * 60 секунд), определено в `src/infrastructure/redis/feedback/constants.py`
...
- Фильтрация происходит в памяти (src/services/courses/query_service_for_talent.py), а не на уровне базы данных
...
- Существует unit-тест `test_timetable_with_warnings` в `unit_tests/domain/timetable/test_entity.py`, но нет интеграционных тестов
- Нет тестов для проверки публикации курса с недостающими обязательными полями
- Нет тестов ограничений на количество отзывов от одного пользователя
